...
model = tf.keras.models.load_model('model/model_2023-11-29', custom_objects={"TFAlbertModel": TFAlbertModel})
...
def predict():
    ...
    # input_ids = tf.TensorSpec.from_tensor(X_test['input_ids'], name = 'input_ids')
    # attention_mask = tf.TensorSpec.from_tensor(X_test['attention_mask'], name = 'attention_mask')
    # predicted = model([X_test['input_ids'], X_test['attention_mask']])

    predicted = model.predict({'input_ids': X_test['input_ids'], 'attention_mask': X_test['attention_mask']})

    y_predicted = [1 if pred >= 0.5 else 0 for pred in predicted]
    print(y_predicted)
    return "{}".format(y_predicted)
